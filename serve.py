...
class App:
    def __init__(self, voice_dir, atlas_dir, profile=None):
        ...
        self._wanted = None   # pck names our banks actually have in the archive
    ...
    def wanted_pcks(self):
        """The .pck names our banks actually have in the archive, resolved once.

        A character has several banks; only some stream from a .pck. Checking the
        index once and caching avoids re-reading data.i on every status poll.
        """
    ...
    def game_was_patched(self, name, backup, live):
        """True only when the live bank is a genuinely new game file.

        Decided by comparing shas, not by replaying intent onto the backup:
        replay cannot reproduce chained swaps, so it used to misread our own
        edits as a patch and retire the user's only original. The file being
        exactly what we last wrote, or exactly the pristine backup, is proof
        nothing external changed it. Anything less keeps the backup, because
        wrongly keeping one costs nothing and wrongly retiring one costs the
        user their original.
        """
        try:
    ...
    def revert_all(self):
        """Undo every character. The button a panicking user actually wants."""
        done, failed = [], {}
        for c in self.characters():
            pl = c["pl"]
    ...
    def _revert(self, pl):
        ...
        for path in self.bank_paths(pl):     # paths only; a corrupt live bank is fine
            ...
            try:
                ...
                # unrecorded, so a later reapply simply skips it (safe) rather than
...
def main():
    ...
    # A busy port must not be a dead end: someone who double-clicked run.bat
    # cannot pass --port, and the printed URL is all they actually need.
